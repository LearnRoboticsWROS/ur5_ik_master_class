...
public:
    ...
    node_(node)
  {
    ...
    // Configure link attacher clients (Gazebo grasp emulation)
  ...
  void close_gripper()
  {
    gripper_.setJointValueTarget("robotiq_85_left_knuckle_joint", 0.2);
    gripper_.move();
  }
  ...
  void attachObject()
  {
    ...
    request->model1_name = "cobot";          // Robot model name in Gazebo
    request->link1_name  = "wrist_3_link";   // Gripper/EE link name
    request->model2_name = "red_cube";       // Object model name in Gazebo
    request->link2_name  = "link_1";         // Object link name

    while (!attach_client_->wait_for_service(std::chrono::seconds(1)))
    {
      RCLCPP_WARN(logger_, "Waiting for the AttachLink service...");
    }
    ...
    {
      RCLCPP_ERROR(logger_, "Failed to attach object.");
    }
  }
  ...
  void detachObject()
  {
    ...
    {
      RCLCPP_WARN(logger_, "Waiting for the DetachLink service...");
    }
    ...
    {
      RCLCPP_ERROR(logger_, "Failed to detach object.");
    }
  }
  ...
  {
    attached_object_.link_name = "wrist_3_link";
    attached_object_.object = collision_objects_[3];
    attached_object_.object.operation = attached_object_.object.ADD;

    // Links allowed to be in contact with the attached object
    attached_object_.touch_links = {
      "robotiq_85_right_finger_tip_link",
      "robotiq_85_left_finger_tip_link",
      "wrist_3_link"
    };

    planning_scene_interface_.applyAttachedCollisionObject(attached_object_);
  }

  void detachCollisionObject()
  {
    attached_object_.link_name = "wrist_3_link";
    attached_object_.object.operation = attached_object_.object.REMOVE;
    planning_scene_interface_.applyAttachedCollisionObject(attached_object_);
  }
...
private:
  ...
  moveit_msgs::msg::AttachedCollisionObject attached_object_;
...
int main(int argc, char** argv)
{
  ...
  // Attach collision object in MoveIt scene (optional)
